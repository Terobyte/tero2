...
- [ ] **Create a worktree off main** (see superpowers:using-git-worktrees):
...
- [ ] **Read the spec once, top to bottom:** `docs/superpowers/specs/2026-04-20-live-agent-stream-design.md`. The Decisions Log (spec §Decisions Log) encodes 10 choices that determine why each task below exists.
...
- `tests/normalizers/test_claude.py`, `test_codex.py`, `test_opencode.py`, `test_kilo.py`, `test_zai.py`
- `tests/normalizers/fixtures/` (raw provider output samples)
...
```
Expected: `ModuleNotFoundError: No module named 'tero2.stream_bus'`.

- [ ] **Step 3: Implement `StreamEvent` + `make_stream_event`**
...
uv run pytest tests/test_stream_bus.py -v
```
Expected: new tests fail with `ImportError: cannot import name 'StreamBus'`.

- [ ] **Step 3: Add `StreamBus` class**
...
- [ ] **Step 4: Register via side-effect import** — add to `tero2/providers/normalizers/__init__.py`:
...
- [ ] **Step 2: Run — expect timeout/fail**

```bash
uv run pytest tests/test_cli_provider_streaming.py -v --timeout=3
```

- [ ] **Step 3: Do NOT commit yet** — Task 12 ships test + refactor in one commit.
...
- [ ] **Step 3:** `uv run pytest tests/ -q` → full green
- [ ] **Step 4: Commit**
...
1. Recoverable errors BEFORE first yield → retry this provider, then fail over.
2. Recoverable errors AFTER first yield → hard fail.
3. Non-recoverable errors → hard fail always.
4. First message of shape `{"type":"error",...}` → treat as pre-yield stream error, raise `ProviderError`.
5. Mid-stream `{"type":"error"}` dict → pass through unchanged (normalizer handles).
...
- [ ] **Step 2:** `uv run pytest tests/ -q` → green.
- [ ] **Step 3: Commit**
...
- [ ] **Step 3: Modify `DashboardApp`**

1. Signature: add `stream_bus`.
2. Replace `compose`:
...
5. Remove `LogView` from `compose` only (keep file on disk).
...
- [ ] **Step 3: Update `BINDINGS` + actions**
...
- [ ] **Step 2:** `uv run pytest tests/ -q` → green (existing tests don't pass `stream_bus`, defaults to `None` on `RunnerContext`).
- [ ] **Step 3: Commit**
