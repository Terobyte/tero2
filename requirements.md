...
- [x] **Python:** 3.11+
- [x] **Platform:** macOS (Darwin)
- [x] **Package manager:** uv (preferred) or pip
...
**Data flow:**
- [x] `cli.py` parses args, loads config, calls `runner.run()`
- [x] `runner.py` reads state from disk, spawns a provider via `ProviderChain`, streams output
- [x] On success → `checkpoint.py` saves state → next task or done → `notifier.py` sends Telegram
- [x] On crash → `checkpoint.py` restores state → retry from last checkpoint
- [x] On rate limit → `chain.py` + `circuit_breaker.py` → fallback to next provider
- [x] On stuck/done → `notifier.py` → Telegram message (text, optionally voice)
...
**Rules:**
- [x] Every constant has a type annotation.
- [x] No imports from other `tero2` modules.
- [x] Group by domain with section comments.
...
- [x] Global: `~/.tero2/config.toml`
- [x] Project: `<project_path>/.sora/config.toml`
...
**Behavior rules:**
- [x] `save()` must be atomic: write to a `.tmp` file then `os.rename()`. This guarantees no partial writes on crash.
- [x] `from_file()` returns default `AgentState()` if file doesn't exist or is corrupt — never crashes.
- [x] All timestamps are ISO 8601 UTC.
- [x] `Phase` is a string enum so it serializes naturally to JSON.
...
**Behavior rules:**
- [x] Use `fcntl.flock(fd, LOCK_EX | LOCK_NB)` for non-blocking exclusive lock.
- [x] Write `PID\n` to the lock file after acquiring.
- [x] On `acquire()`: if `flock` fails with `EAGAIN`/`EACCES`, read PID from file, check if process is alive (`os.kill(pid, 0)`). If dead → remove stale lock and retry once. If alive → raise `LockHeldError`.
- [x] `release()` removes the lock file. Silent if already released.
- [x] Lock file path: `.sora/runtime/auto.lock`
...
**Behavior rules:**
- [x] All write operations create parent directories automatically (`Path.mkdir(parents=True, exist_ok=True)`).
- [x] All reads return empty string/dict for missing files — never raise.
- [x] `write_state()` uses atomic write (temp file + rename).
- [x] `append_activity()` opens file in append mode, writes one JSON line, closes immediately.
- [x] Methods that read from `human/` are called by the runner at phase boundaries.
...
**Import path changes:**
- [x] v1: `from src.constants import X` → tero2: `from tero2.constants import X`
- [x] v1: `from src.errors import X` → tero2: `from tero2.errors import X`
- [x] v1: `from src.config import get_context_window` → tero2: remove dependency (only in `zai.py`)
...
- [x] `ruff check tero2/providers/`
- [x] All imports resolve
- [x] `python -c "from tero2.providers import create_provider"` succeeds
...
**Behavior rules:**
- [x] **Never raise exceptions.** Notification failure must not crash the runner. Log and return False.
- [x] `notify()` always sends text. Adds voice for `DONE` (if `voice_on_done`) and `STUCK` (if `voice_on_stuck`).
- [x] Voice uses `tts_fish_audio_simple()` from existing library. Import dynamically to avoid hard dependency.
- [x] Telegram Bot API: `POST https://api.telegram.org/bot{token}/sendMessage` for text, `sendVoice` for audio.
- [x] Use `requests` library (sync is fine — notifications are fire-and-forget).
...
**Behavior rules:**
- [x] Every `mark_*` and `increment_*` method calls `self.disk.write_state()` internally.
- [x] `save()` sets `last_checkpoint` to current UTC ISO timestamp.
- [x] All methods return the updated state (for chaining).
- [x] `restore()` never raises — returns default state if disk is empty.
- [x] `mark_running()` is the inverse of `mark_paused()` — used when PAUSE directive is cleared.
...
- [x] The retry backoff (60s wait between chain retries) gives providers time to recover.
- [x] The runner's 3-attempt limit still applies — it won't loop forever.
...
- [x] `tero2 init ~/testproject` creates `.sora/` directory structure
- [x] `tero2 run ~/testproject --plan plan.md` starts the agent and executes the plan
