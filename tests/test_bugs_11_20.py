...
class TestBug11OffByOneToolRepeatThreshold:
    ...
    def test_update_tool_hash_triggers_prematurely(self):
        config = StuckDetectionConfig(tool_repeat_threshold=2)
        state = AgentState()
        ...
        assert result.signal != StuckSignal.TOOL_REPEAT, (
            ...
            f"threshold=2. check_stuck fires TOOL_REPEAT via `1 >= 2-1`. "
            f"Should only fire after 3+ identical calls (count >= threshold)."
        )

    def test_source_uses_exact_threshold_not_minus_one(self):
        ...
        assert "tool_repeat_count >= config.tool_repeat_threshold" in tool_section, (
            "check_stuck uses `>= threshold - 1` which is off-by-one. "
            "Should use `tool_repeat_count >= config.tool_repeat_threshold` "
            "so that threshold=2 fires after 2+ repeats, not 1."
        )
